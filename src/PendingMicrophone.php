...
class PendingMicrophone
{
    ...
    {
        \Log::debug('PendingMicrophone::start() called', ['started' => $this->started]);

        if ($this->started) {
            \Log::debug('PendingMicrophone already started, returning false');
        ...
        if (function_exists('nativephp_call')) {
            ...
            \Log::debug('Calling Microphone.Start', ['payload' => $payload]);
            $result = nativephp_call('Microphone.Start', json_encode($payload));
            \Log::debug('Microphone.Start result', ['result' => $result]);

            if ($result) {
                $decoded = json_decode($result, true);
                \Log::debug('Microphone.Start decoded', ['decoded' => $decoded]);
                ...
                \Log::debug('Microphone.Start success status', ['success' => $success, 'isError' => $isError]);

                return $success;
            }
        } else {
            \Log::error('nativephp_call function does not exist');
        }

        \Log::debug('PendingMicrophone::start() returning false');
    ...
    {
        \Log::debug('PendingMicrophone::__destruct() called', ['started' => $this->started]);

        if (! $this->started) {
            \Log::debug('Auto-starting microphone recording in __destruct()');
            $this->start();
        }
    }
}